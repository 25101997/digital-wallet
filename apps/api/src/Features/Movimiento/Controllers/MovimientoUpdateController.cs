...
public class MovimientoUpdateController : ControllerBase
{
    private readonly PostgreSqlConnectionFactory _connectionFactory;

    public MovimientoUpdateController(PostgreSqlConnectionFactory connectionFactory)
    {
        _connectionFactory = connectionFactory;
    }

    [HttpPut("{idMovimiento:int}")]
    public async Task<IActionResult> Update(
        int idMovimiento,
        [FromBody] MovimientoUpdateDTO dto
    )
    {
        if (idMovimiento <= 0)
        ...
        try
        {
            ...
            const string sql = @"
                SELECT dw.actualizar_movimiento(
                    @p_id_movimiento,
                    @p_id_cuenta,
                    @p_tipo,
                    @p_monto,
        ...
        catch (Exception ex)
        {
            ...
            {
                mensaje = "Error interno al actualizar el movimiento.",
                detalle = ex.Message
            });
        }
    }
